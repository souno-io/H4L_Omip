...
class H4LBaseModel(models.Model):
    ...
    create_datetime = models.DateTimeField(_("创建时间"), auto_now_add=True, null=True, blank=True,
                                           help_text="创建时间")
...
def get_all_models_objects(model_name=None):
    ...
    :return: {}
    """
    if not hasattr(settings, 'ALL_MODELS_OBJECTS') or not settings.ALL_MODELS_OBJECTS:
        ...
        for item in all_models:
            table = {
                ...
                "table": item.__name__,
                "tableFields": [
                    {"title": field.verbose_name, "field": field.name} for field in item._meta.fields
                ]
            }
            settings.ALL_MODELS_OBJECTS[item.__name__] = {"table": table, "object": item}

    if model_name:
        return settings.ALL_MODELS_OBJECTS.get(model_name, {})
    return settings.ALL_MODELS_OBJECTS
