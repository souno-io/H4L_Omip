  ...
  methods: {
    ...
    listSort(sKey, eKey) {
      ...
      this.list.forEach((item) => {
        if (item.sortKey === eKey)
          endRealKey = item.realKey;
      });
    ...
    touchmove(e) {
      if (!this.isDrag || !this.tDragging)
        return;
      let touch = e.changedTouches[0] || e.touches[0];
      if (!touch || this.sId !== touch.identifier)
        return;
      ...
      if (this.outOfRange(curX, Number(this.columns), curY, this.rows, endKey, this.list.length))
        return;
      if (startKey === endKey || startKey === this.preStartKey)
        return;
