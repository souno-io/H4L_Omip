...
const mpjs = {
  ...
  methods: {
    cutDetectionPosition() {
      var windowHeight = this.windowHeight, windowWidth = this.windowWidth;
    ...
    touchmove(e) {
      var touch = e.touches || e.changedTouches;
      if (this.flagEndTouch)
        return;
      this.moveDuring();
      if (touch.length == 1) {
        var left = touch[0].clientX - this.touchRelative[0].x, top = touch[0].clientY - this.touchRelative[0].y;
        this.imgLeft = left;
        this.imgTop = top;
        this.imgMarginDetectionPosition();
      } else {
        var width = Math.abs(touch[0].clientX - touch[1].clientX), height = Math.abs(touch[0].clientY - touch[1].clientY), hypotenuse = Math.sqrt(Math.pow(width, 2) + Math.pow(height, 2)), scale = this.scale * (hypotenuse / this.hypotenuseLength), current_deg = 0;
