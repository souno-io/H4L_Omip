<!--本文件由FirstUI授权予叶兴（手机号：18 7 9    8 79   1549，身份证尾号： 05 5  9 16）专用，请尊重知识产权，勿私下传播，违者追究法律责任。-->
<template>
		...
		<view class="fui-preview__bd" v-if="pvd[list] && pvd[list].length>0"
			...
			<view class="fui-preview__item" v-for="(item,index) in pvd[list]" :key="index">
				<text class="fui-preview__label"
					...
					:style="{color:item[lColor] || labelColor,fontSize:bdSize+'rpx',width:labelWidth>0?labelWidth+'rpx':'auto'}">{{item[label]}}</text>
